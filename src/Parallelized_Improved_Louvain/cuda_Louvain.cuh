#ifndef CUDA_LOUVAIN_CUH
#define CUDA_LOUVAIN_CUH
 ...
 * 1. Graph data stored on GPU in CSR format
 * 2. Parallel modularity delta computation for all nodes
 * 3. Asynchronous node moves with conflict resolution
 * 4. GPU-based community degree updates using atomics
 * 5. Parallel modularity computation
 */

namespace cuda_louvain {
...
// Device graph structure (CSR format on GPU)
struct DeviceGraph {
    EdgeID* d_row_ptr;
    NodeID* d_col_idx;
    Weight* d_weights;
...
// Kernel: Compute modularity delta for best community for each node
__global__ void compute_best_moves_kernel(
    ...
    // Use shared memory for neighbor community weights
    extern __shared__ Weight shared_mem[];
    
    // Compute weights to neighboring communities
    // We'll use a simple approach: iterate neighbors and accumulate
    Weight k_i_in_current = 0.0f;
    
    EdgeID start = row_ptr[node];
    EdgeID end = row_ptr[node + 1];
    
    // First pass: find k_i_in for current community
    ...
    // Second pass: check each unique neighbor community
    for (EdgeID e = start; e < end; ++e) {
        NodeID neighbor = col_idx[e];
        Community neighbor_comm = node_to_comm[neighbor];
...
// Kernel: Apply node moves with conflict resolution
...
// Kernel: Mark active nodes (nodes that moved + their neighbors)
__global__ void mark_active_nodes_kernel(
    const EdgeID* __restrict__ row_ptr,
    const NodeID* __restrict__ col_idx,
...
// Kernel: Detect tree structures using BFS
__global__ void detect_trees_kernel(
    const EdgeID* __restrict__ row_ptr,
    const NodeID* __restrict__ col_idx,
    NodeID num_nodes,
    bool* is_tree_node,
    ...
    // This is complex to parallelize - typically done on CPU
    // Simplified version: mark small degree-1 chains
    NodeID node = blockIdx.x * blockDim.x + threadIdx.x;
    if (node >= num_nodes) return;
    
    EdgeID degree = row_ptr[node + 1] - row_ptr[node];
    
    // Simple heuristic: degree-1 nodes are likely in trees
    if (degree == 1) {
...
private:
    void upload_graph_to_device(const Graph& graph) {
        ...
        size_t row_ptr_size = (graph.num_nodes() + 1) * sizeof(EdgeID);
        size_t col_idx_size = 0;
        size_t weights_size = 0;
        
        // Get sizes from graph
        for (NodeID i = 0; i < graph.num_nodes(); ++i) {
            col_idx_size += graph.num_neighbors(i);
            weights_size += graph.num_neighbors(i);
        }
        col_idx_size *= sizeof(NodeID);
        weights_size *= sizeof(Weight);
        ...
        CUDA_CHECK(cudaMalloc(&d_graph_.d_node_degrees, graph.num_nodes() * sizeof(Weight)));
        ...
        CUDA_CHECK(cudaMemcpy(d_graph_.d_node_degrees, h_degrees.data(),
                             graph.num_nodes() * sizeof(Weight), cudaMemcpyHostToDevice));
    }
    ...
    void initialize_communities(NodeID num_nodes) {
        ...
        // All nodes initially active
        thrust::fill(d_active_.begin(), d_active_.end(), true);
    }
    
    bool phase1_iteration() {
        NodeID num_nodes = d_graph_.num_nodes;
        int num_blocks = (num_nodes + block_size_ - 1) / block_size_;
        
        // Reset improvement flags
        ...
        compute_best_moves_kernel<<<num_blocks, block_size_>>>(
            d_graph_.d_row_ptr,
            d_graph_.d_col_idx,
            d_graph_.d_weights,
            d_graph_.d_node_degrees,
